...
def writeProcesses(f,d,options):
  ...
  for cat in d.cat.unique():
	...
	lprocid += "%-55s "%sigID
	sigID -= 1
      elif r['proc'] == "bkg_mass": lprocid += "%-55s "%"1"
      else:
	print "Proc %s bkgId %s"%(r['proc'],bkgID)
	bkgID += 1
        lprocid += "%-55s "%bkgID
#        bkgID += 1
...
def writeProcScaler(f,d,options):
  f.write("\n")
  for cat in d[~d['cat'].str.contains("NOTAG")].cat.unique():
#    for ir,r in d[d['cat']==cat].iterrows(): 
#        if "Tvlq" in r['proc']: 	
     scaler1 = "ttHScaler rateParam %s ttH_* 0.48036"%cat
     scaler2 = "TvlqScaler rateParam %s Tvlq* 0.93157"%cat
     f.write("%s\n"%scaler1)
     f.write("%s\n"%scaler2)
  f.write("\n")
  f.write("nuisance edit freeze ttHScaler\n")
  f.write("nuisance edit freeze TvlqScaler")
  return True   
def writeBreak(f):
  lbreak = '----------------------------------------------------------------------------------------------------------------------------------'
  f.write("%s\n"%lbreak)
