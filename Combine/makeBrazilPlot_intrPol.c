...
void makeBrazilPlot_intrPol(TString channel="", TString method="mu", TString ext=""){
gStyle->SetOptStat(0);
gROOT->SetBatch();
const int points=15;
int mass[points]	={600,	 	625, 	650, 	675, 	700, 	750,	800, 	850,	900, 	950,	1000, 	1050,	1100, 	1150,	1200};
float T_xs[points] 	={0.1764,	0.1489,	0.1213,	0.1050,	0.0886,	0.0627,	0.0459,	0.0339,	0.0251,	0.0189,	0.0145, 0.0112,	0.00867,0.00679,0.00536};
/*
...
TString file_path="/afs/cern.ch/work/p/prsaha/public/FinalFit_lite/CMSSW_10_2_13/src/flashggFinalFit/Combine/result_v13/Models_Tprime";
float lumi=137;
double x[points];
double y[points];
...
for(int i=0; i<points; i++)
	...
	if(method=="xs_X_Br")  xs_X_Br_ = T_xs[i]*1000*0.00223; //in fb
        ...
        ss << mass[i];
        ss >> mass_str;
        TFile *file_ = new TFile(file_path + mass_str+ channel +"/higgsCombine" + channel + "_StatOnly.AsymptoticLimits.mH125.root", "READ");
...
//====================================================================================================
TString file_path_Stat="/afs/cern.ch/work/p/prsaha/public/FinalFit_lite/CMSSW_10_2_13/src/flashggFinalFit/Combine/result_v13/Models_Tprime";
...
//cout<<"nentries=========="<<nentries<<endl;
        ...
        for(int ievent=0; ievent <points ; ievent++)
                ...
                tree_-> GetEntry( ievent );
//                x[ievent] = mass[i];
                if (ievent==2) {y_Stat[i]= qlimit*xs_X_Br_; cout<<"Tprime"<<mass[i]<<" mean =   "<<qlimit<<endl;}
//      else    cout<<"Limit ---  "<<qlimit<<endl;
            }
//cout<<"y[i]   "<<y[i]<<endl;
...
//ge2->GetYaxis()->SetRangeUser(0, 35);

ge2->GetYaxis()->SetRangeUser(0, 150);
//ge2->GetYaxis()->SetRangeUser(0, 3.5);
...
g_obs->SetLineStyle(2);
        ...
        TLatex * tex2 = new TLatex(0.76, 0.95,TString(Form("%.1f fb^{-1} (13 TeV)",lumi)));
        tex2->SetNDC();
...
leg1->AddEntry(g,  "Expected(H #rightarrow #gamma #gamma)(Stat)"		,"l");
...
c41->SetGridy();
ge2->Draw("a3");
ge2->GetXaxis()->SetNdivisions(20);
ge2->GetYaxis()->SetNdivisions(20);
ge->Draw("Same3l");
g->Draw("SAME");
//g_Stat->Draw("SAME");
//graph->Draw("SAME");
...
c41->SaveAs("/eos/user/p/prsaha/www/Tprime_analysis/Finalfit_Lite_output/result_v13/LimitTprime_"+ method + channel + ext + ".pdf");
c41->SaveAs("/eos/user/p/prsaha/www/Tprime_analysis/Finalfit_Lite_output/result_v13/LimitTprime_"+ method + channel + ext + ".png");
c41->SaveAs("/eos/user/p/prsaha/www/Tprime_analysis/Finalfit_Lite_output/result_v13/LimitTprime_"+ method + channel + ext + ".root");
}
