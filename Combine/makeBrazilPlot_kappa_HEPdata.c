...
void makeBrazilPlot_kappa_HEPdata(TString channel="", TString method="mu", TString ext=""){
gStyle->SetOptStat(0);
gROOT->SetBatch();
setTDRStyle();
const int points=8;
...
if(method=="xs_X_Br") {
    ...
    ge2->SetMaximum(0.7);
    ge2->SetMinimum(0.05);
	ge2->GetYaxis()->SetTitle("#scale[0.75]{T'- SM particles coupling strength}");
    }
...
//    cout<<"test=\t"<<MQ_<<"\t"<<width_<<"\t"<<kT<<endl;
	if(width_==5)cout<<"  - value: "<<kT<<endl;
    x_[j]=MQ_;
    y_[j]=kT;
    j++;
    }
...
tex1->DrawLatex(0.25, 0.335, "#scale[0.6]{#Gamma/M_{T'}=1%}");
tex1->DrawLatex(0.25, 0.41, "#scale[0.6]{#Gamma/M_{T'}=2%}");
tex1->DrawLatex(0.25, 0.47, "#scale[0.6]{#Gamma/M_{T'}=3%}");
tex1->SetTextAngle(-23);
tex1->DrawLatex(0.25, 0.54, "#scale[0.6]{#Gamma/M_{T'}=4%}");
tex1->DrawLatex(0.25, 0.6, "#scale[0.6]{#Gamma/M_{T'}=5%}");
tex1->SetTextAngle(0);
