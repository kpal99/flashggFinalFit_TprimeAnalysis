...
void makeBrazilPlot_Xs_ATLAScomp(TString channel="", TString method="mu", TString ext=""){
gStyle->SetOptStat(0);
gROOT->SetBatch();
setTDRStyle();
const int points=10;
...
Double_t Th_xs_tHbbATLAS_x[3] = {
    1000,
    1100,
    1200};
Double_t Th_xs_tHbbATLAS_y[3] = {
    6.7,
    3.8,
    2.65
    };

   	TGraph *Th_xs_tHbbATLAS = new TGraph(3, Th_xs_tHbbATLAS_x, Th_xs_tHbbATLAS_y);
   	Th_xs_tHbbATLAS->SetName("expXsec");
   	Th_xs_tHbbATLAS->SetTitle("");
   	Th_xs_tHbbATLAS->SetFillStyle(1000);
   	Th_xs_tHbbATLAS->SetLineWidth(2);
//	Th_xs_tHbbATLAS->SetLineStyle(2);
   	Th_xs_tHbbATLAS->SetLineColor(kMagenta);

Double_t xs_tHbbATLAS_x[3] = {
    1000,
    1100,
    1200};
Double_t xs_tHbbATLAS_y[3] = {
    235,
    33,
    32
    };

   TGraph *xs_tHbbATLAS = new TGraph(3, xs_tHbbATLAS_x, xs_tHbbATLAS_y);
   xs_tHbbATLAS->SetName("expXsec");
   xs_tHbbATLAS->SetTitle("");
   xs_tHbbATLAS->SetLineStyle(2);
   xs_tHbbATLAS->SetLineWidth(2);
   xs_tHbbATLAS->SetLineColor(kMagenta);

TLegend *leg1 = new TLegend(0.43, 0.75, 0.91, 0.88, "");
...
leg1->AddEntry(Th_kTVsXs_fkT2[2],"#scale[1.1]{Theory at fixed #kappa_{T}}" ,"l");
//leg1->AddEntry(Th_WidthVsXs,"#scale[1.1]{Theory at fixed #Gamma/M_{T'}=5\%}" ,"l");
leg1->AddEntry(xs_tHbbATLAS,"#scale[1.1]{Expected (ATLAS, #kappa_{T}=0.1)}" ,"l");
//leg1->AddEntry(Th_xs_tHbbATLAS,"#scale[1.1]{Theory (ATLAS, #kappa_{T}=0.1)}" ,"l");
...
//Th_line->Draw("CSAME");
//mg->Draw("CSAME");
//Th_kTVsXs[0]->Draw("CSAME");
//Th_kTVsXs[1]->Draw("CSAME");
/*Th_kTVsXs[2]->Draw("CSAME");
Th_kTVsXs[3]->Draw("CSAME");
Th_kTVsXs[4]->Draw("CSAME");
Th_kTVsXs[5]->Draw("CSAME");
Th_kTVsXs[6]->Draw("CSAME");
*/
...
//Th_xs_tHbbATLAS->Draw("CSAME");
xs_tHbbATLAS->Draw("CSAME");

tex1->SetTextColor(kRed);
//tex1->SetTextAngle(-17);
//tex1->DrawLatex(0.3, 0.21, "#scale[0.6]{#kappa_{T}=0.05}");
tex1->SetTextAngle(-13);
tex1->DrawLatex(0.77, 0.31, "#scale[0.6]{#kappa_{T}=0.10}");
tex1->SetTextAngle(-12);
tex1->DrawLatex(0.77, 0.41, "#scale[0.6]{#kappa_{T}=0.15}");
tex1->DrawLatex(0.77, 0.47, "#scale[0.6]{#kappa_{T}=0.20}");
tex1->DrawLatex(0.77, 0.52, "#scale[0.6]{#kappa_{T}=0.25}");

/*tex1->SetTextAngle(-17);
tex1->DrawLatex(0.2, 0.22, "#scale[0.6]{#kappa_{T}=0.05}");
tex1->SetTextAngle(-15);
tex1->DrawLatex(0.2, 0.245, "#scale[0.6]{#kappa_{T}=0.10}");
tex1->SetTextAngle(-27);
tex1->DrawLatex(0.2, 0.305, "#scale[0.6]{#kappa_{T}=0.15}");
tex1->SetTextAngle(-30);
tex1->DrawLatex(0.2, 0.42, "#scale[0.6]{#kappa_{T}=0.20}");
tex1->SetTextAngle(-35);
tex1->DrawLatex(0.2, 0.57, "#scale[0.6]{#kappa_{T}=0.25}");
*/
//tex1->DrawLatex(0.77, 0.56, "#scale[0.6]{#kappa_{T}=0.30}");
//tex1->DrawLatex(0.77, 0.61, "#scale[0.6]{#kappa_{T}=0.35}");
...
tex1->DrawLatex(0.25, 0.89, "#scale[0.8]{T' #rightarrow tH(H #rightarrow #gamma #gamma)}");
