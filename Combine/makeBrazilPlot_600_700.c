...
void makeBrazilPlot_600_700(TString channel="", TString method="mu", TString ext=""){
gStyle->SetOptStat(0);
gROOT->SetBatch();
const int points=5;
...
for(int i=0; i<points; i++)
        {
	double xs_X_Br_=1;
	if(method=="xs_X_Br")  xs_X_Br_ = T_xs[i]*0.00227; //in fb
	else xs_X_Br_ =1;
        string mass_str;
        stringstream ss;
        ss << mass[i];
...
//cout<<"nentries=========="<<nentries<<endl;
	...
	x[i] = mass[i];
        for(int ievent=0; ievent <points+1 ; ievent++)
                {
...
//====================================================================================================
TString file_path_Stat="/afs/cern.ch/work/p/prsaha/public/FinalFit_lite/CMSSW_10_2_13/src/flashggFinalFit/Combine/Models_Tprime";
//TString file_path_Stat="/eos/user/p/prsaha/Tprime_analysis/FinalFit_outputs/result_v13/Models_Tprime";
for(int i=0; i<points; i++)
        {
    double xs_X_Br_=1;
    if(method=="xs_X_Br")  xs_X_Br_ = T_xs[i]*0.00223; //in fb
    else xs_X_Br_ =1;
        string mass_str;
        stringstream ss;
        ss << mass[i];
...
c41->SaveAs("/eos/user/p/prsaha/www/Tprime_analysis/Finalfit_Lite_output/UL/result_v2_MidPurity/LimitTprime_"+ method + channel + ext + ".pdf");
c41->SaveAs("/eos/user/p/prsaha/www/Tprime_analysis/Finalfit_Lite_output/UL/result_v2_MidPurity/LimitTprime_"+ method + channel + ext + ".png");
c41->SaveAs("/eos/user/p/prsaha/www/Tprime_analysis/Finalfit_Lite_output/UL/result_v2_MidPurity/LimitTprime_"+ method + channel + ext + ".root");
}
