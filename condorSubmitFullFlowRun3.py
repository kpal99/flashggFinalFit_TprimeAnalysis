...
def condorSubmitFullFlow(args):
    ...
    # Paths
    jobFileBaseName = f"fullFlow_Run3"
    ...
    with open(sh_path, "w") as sh:
        ...
        for idx, (mass, width) in enumerate(combinations):
            sh.write(f"\nif [ $1 -eq {idx} ]; then\n")
            with open("fullFlowRun3.jinja2") as template:
                jinjaOutput = jinja2.Template(template.read()).render(
                    pwd=current_working,
                    finalSelectionDir2022=args.finalSelectionDir2022,
                    finalSelectionDir2023=args.finalSelectionDir2023,
...
def main():
    ...
    parser.add_argument('--finalSelectionDir2022', required=True, help="Final selection directory i.e finalSelectionXp6-22")
    parser.add_argument('--finalSelectionDir2023', required=True, help="Final selection directory i.e finalSelectionXp6-23")
    parser.add_argument('--finalSelectionDirData', required=True, help="Final selection directory i.e finalSelectionXp6-22plus23")
